...
`Procfile` приведён к текущему стеку: `release` прогоняет миграции, `web` стартует
собранное приложение. Docker на PaaS обычно недоступен, поэтому серверное
исполнение кода там деградирует (см. выше).
