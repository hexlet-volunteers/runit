...
const LIMITS = {
   ...
   * запросов между адресами. Поэтому в дополнение к нему в authRouter.login
   * (#858) есть второй, независимый рубеж — счётчик неудач на конкретный
   * email в таблице login_attempts (см. src/auth/bruteforce.ts): его
   * распределением запросов по IP не обойти, потому что ключ — не адрес
   * клиента, а сам логин, который пытаются подобрать.
   */
