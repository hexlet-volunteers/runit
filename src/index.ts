...
const getApp = async () => {
  ...
  /**
   * Предел размера тела запроса — первая линия защиты от простого способа
   * занять память сервера. Значение чуть больше самого крупного законного тела
   * (аватар до 700 000 символов base64 плюс служебные поля), чтобы отказ
   * получали только заведомо ненормальные запросы.
   *
   * Fastify по умолчанию ставит 1 МБ, и это работало, но молча: предел аватара
   * был выше лимита тела, поэтому крупная картинка получала 413 вместо
   * понятного сообщения. Теперь оба числа заданы явно и согласованы.
   */
