...
export default defineConfig({
  ...
  server: {
    ...
    open: "/",
    /**
     * Кто может обращаться к dev-серверу из браузера.
     *
     * По умолчанию Vite отвечает `Access-Control-Allow-Origin` на любой origin.
     * Вместе с проксированием /trpc это означало: пока разработчик залогинен в
     * локальном приложении, ЛЮБОЙ открытый им сайт мог прочитать ответы нашего
     * API от его имени — проверено экспериментом, страница на другом порту
     * получила почту пользователя из auth.me. Бэкенд и прод-сборка такой запрос
     * отвергают (там CORS_ORIGIN), дыра была только в дев-режиме.
     */
