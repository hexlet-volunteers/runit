 ...
 */
/**
 * ВНИМАНИЕ, требование 152-ФЗ. Отчёты об ошибках содержат IP-адрес и адрес
 * страницы, то есть персональные данные. Облачный sentry.io размещён за
 * пределами РФ, а запись и хранение персональных данных граждан РФ в базах
 * за пределами страны не допускаются (ст. 18 ч. 5) — плюс это была бы
 * трансграничная передача, которую Политика прямо отрицает.
 *
 * Поэтому в прод VITE_SENTRY_DSN можно задавать только для self-hosted Sentry,
 * развёрнутого в РФ (#868). Пустая переменная отключает отправку целиком.
 */
