...
# CLI Docker — рабочий инструмент раннера, а не отладочная утилита: код
# сниппетов исполняется командой `docker run`, и без бинарника серверное
# исполнение недоступно в принципе. Раньше образ его не содержал, а раннер
# отвечал «Docker CLI не найден» — то есть девять из двенадцати языков в проде
# не работали, притом что подключить сокет мы предлагали в docs/runner.md.
# Берётся готовый статический бинарник из официального образа: apt-тянет
# демон и systemd, которые контейнеру не нужны.
